...
// /testbookmarks — saved novels, redesigned to the /test* standard. Reuses the real
    ...
    <div className="tnl-root relative min-h-screen text-white" style={{ ['--v' as string]: '124,58,237' }}>
      ...
      <main className="relative z-10 mx-auto max-w-[1400px] px-4 pb-24 pt-8 sm:px-6">
        <div className="mb-6 flex items-end justify-between gap-4">
          ...
          <Link href="/testbrowse" className="shrink-0 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white/80 transition hover:text-white">Browse more →</Link>
        </div>
        ...
        {!mounted ? (
          ...
          <div className="tnl-panel flex flex-col items-center justify-center gap-4 py-24 text-center">
            ...
            <Link href="/testbrowse" className="mt-1 rounded-xl px-5 py-2.5 text-sm font-bold transition hover:brightness-110" style={{ background: 'rgb(var(--v))' }}>Browse novels</Link>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            {books.map(b => (
              <div key={b.slug} className="group relative">
                <Link href={`/testnewlibrary/${b.slug}`} className="tnl-sheen block">
                ...
                <Link href={`/testnewlibrary/${b.slug}`} className="mt-1.5 block line-clamp-2 text-[13px] font-semibold leading-tight transition hover:text-[rgb(var(--v))]">{b.title}</Link>
