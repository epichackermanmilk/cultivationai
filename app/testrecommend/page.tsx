...
// /testrecommend — AI recommendations, restyled to match the /test* redesign.
// Same functionality as /recommend (novel-based or description-based, spoiler-safe
// toggle, /api/recommend) with the dark-glass purple theme + TestHeader. Signed-out
// users are pushed to /testlogin?return=/testrecommend.

import { useState, useEffect, useRef } from 'react'
import Link from 'next/link'
import TestHeader from '@/components/TestHeader'
import { TestStyles, type Novel } from '@/components/TestUI'
...
function NovelPicker({ library, selected, onAdd, onRemove }: {
  ...
  const q = query.trim()
  const filtered = (q ? library.filter(n => !sel.has(n.slug) && (matchesSearch(n.title, q) || matchesSearch(n.author, q)))
    : library.filter(n => !sel.has(n.slug))).slice(0, 8)
  useEffect(() => {
    ...
    <div ref={wrap} className="relative">
      {selected.length > 0 && (
        <div className="mb-3 flex flex-wrap gap-2">
          {selected.map(n => (
            <span key={n.slug} className="flex items-center gap-1.5 rounded-full border border-[rgba(var(--v),0.4)] bg-[rgba(var(--v),0.12)] px-3 py-1 text-xs font-medium" style={{ color: 'rgb(var(--v))' }}>
              {n.title.length > 30 ? n.title.slice(0, 28) + '…' : n.title}
              <button onClick={() => onRemove(n.slug)} className="ml-0.5 opacity-60 transition hover:opacity-100">×</button>
            </span>
          ))}
        </div>
      )}
      {selected.length < 5 ? (
        <div className="relative">
          <input value={query} onChange={e => { setQuery(e.target.value); setOpen(true) }} onFocus={() => setOpen(true)}
            ...
            className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white placeholder-white/35 outline-none transition focus:border-[rgba(var(--v),0.6)]" />
          {open && filtered.length > 0 && (
            <div className="absolute z-30 mt-1 w-full overflow-hidden rounded-xl border border-white/10 bg-[#100d1c] shadow-2xl">
              {filtered.map(n => (
                <button key={n.slug} onClick={() => { onAdd(n); setQuery(''); setOpen(false) }}
                  ...
                  <img src={coverSrc(n.cover_url)} alt="" className="h-10 w-7 shrink-0 rounded object-cover" onError={e => { (e.target as HTMLImageElement).style.visibility = 'hidden' }} />
...
function RecCard({ rec }: { rec: Rec }) {
  return (
    <div className="tnl-panel flex gap-4 p-4">
      <Link href={`/testnewlibrary/${rec.slug}`} className="shrink-0">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={coverSrc(rec.cover_url)} alt={rec.title} className="h-32 w-[88px] rounded-lg object-cover shadow-lg transition hover:scale-105" />
      </Link>
      <div className="flex min-w-0 flex-col gap-1.5">
        <div>
          <Link href={`/testnewlibrary/${rec.slug}`}><h3 className="line-clamp-2 text-sm font-semibold leading-snug transition hover:text-[rgb(var(--v))]">{rec.title}</h3></Link>
          <p className="text-xs text-white/50">{rec.author}</p>
        </div>
        <div className="flex flex-wrap gap-1">
          ...
          <span className="rounded-full border border-white/10 px-2 py-0.5 text-[10px] text-white/55">{(rec.total_chapters || 0).toLocaleString()} ch</span>
        </div>
        <p className="text-xs italic leading-relaxed text-white/55">&ldquo;{rec.blurb}&rdquo;</p>
        <Link href={`/testnewlibrary/${rec.slug}`} className="mt-auto w-fit rounded-lg border border-[rgba(var(--v),0.4)] bg-[rgba(var(--v),0.12)] px-3 py-1 text-xs font-semibold transition hover:brightness-110" style={{ color: 'rgb(var(--v))' }}>View Novel →</Link>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="tnl-root relative min-h-screen text-white" style={{ ['--v' as string]: '124,58,237' }}>
      <div className="pointer-events-none fixed inset-0 -z-10" style={{ background: '#07060d' }}>
        <div className="absolute inset-0" style={{ background: 'radial-gradient(90% 55% at 50% -10%, rgba(var(--v),0.20) 0%, transparent 55%)' }} />
      </div>
      ...
      <main className="relative z-10 mx-auto max-w-3xl px-4 pb-24 pt-12 sm:px-6">
        <div className="mb-10 text-center">
          <h1 className="mb-3 text-3xl font-black tracking-tight sm:text-4xl">Discover Your Next Novel</h1>
          <p className="text-sm text-white/55">AI-powered recommendations · <span className="font-medium" style={{ color: 'rgb(var(--v))' }}>10 tokens</span> per search</p>
        </div>

        <div className="mb-6 flex gap-1 rounded-xl p-1 tnl-panel">
          {([['novels', 'Based on novels I like'], ['description', 'Describe what I want']] as const).map(([m, label]) => (
            <button key={m} onClick={() => { setMode(m); setResults(null); setError(null) }}
              className={`flex-1 rounded-lg py-2.5 text-sm font-medium transition ${mode === m ? 'text-white' : 'text-white/55 hover:text-white'}`}
        ...
        <div className="mb-6 tnl-panel p-5">
          {mode === 'novels' ? (
            <>
              <label className="mb-3 block text-sm font-medium">Select up to 5 novels you enjoy</label>
              {libLoading ? <div className="h-12 animate-pulse rounded-xl bg-white/5" /> :
                <NovelPicker library={library} selected={selected}
                  onAdd={n => setSelected(p => [...p, n])} onRemove={slug => setSelected(p => p.filter(n => n.slug !== slug))} />}
            </>
          ) : (
            <>
        ...
        {results !== null && (
          <div className="mt-8">
            ...
            {results.length === 0 ? (
              ...
              <div className="flex flex-col gap-4">{results.map(rec => <RecCard key={rec.slug} rec={rec} />)}</div>
            )}
            <button onClick={getRecommendations} disabled={loading}
