...
export default function AdUnit({ placement, className, refreshKey, forceNetwork = false }: {
  placement: Placement; className?: string; refreshKey?: string | number; forceNetwork?: boolean
}) {
  const { user } = useAuth()
  ...
  // `forceNetwork` opts a slot out (e.g. games show real Adsterra inventory).
  if (AD_MODE === 'house' && !forceNetwork) return <HouseAd className={className} />
